...
public class SetStatePendingVisitor extends Visitor
{
	public static final String VERB = "SET_STATE_PENDING";

	private ClientFrame frame;

	public SetStatePendingVisitor(ClientFrame frame, String[] parts, Runnable ioException)
	{
		...
		for(int i = 0; i < tableModel.getRowCount(); i++)
		    for(int j = 0; j < tableModel.getColumnCount(); j++)
		    {
		        ...
		        if(val.equals(buddyName))
		        {
					...
					colorComp.addAttributeSet(new HTMLAttributeSet("color", "gray"));
					colorComp.setText("pending");
					...
					if(frame.getBuddyTable().get(buddyName) != null)
					    frame.getBuddyTable().get(buddyName).setState(Buddy.PENDING);
					break;
				}
			}
	}
