...
public class SetStateOfflineVisitor extends Visitor
{
	public static final String VERB = "SET_STATE_OFFLINE";

	private ClientFrame frame;

	public SetStateOfflineVisitor(ClientFrame frame, String[] parts, Runnable ioException)
	{
		...
		for(int i = 0; i < tableModel.getRowCount(); i++)
			for(int j = 0; j < tableModel.getColumnCount(); j++)
			{
				...
				if(val.equals(buddyName))
				{
					...
					colorComp.addAttributeSet(new HTMLAttributeSet("color", "red"));
					colorComp.setText("offline");
					...
					if(frame.getBuddyTable().get(buddyName) != null)
					    frame.getBuddyTable().get(buddyName).setState(Buddy.OFFLINE);
					break;
				}
			}
	}
